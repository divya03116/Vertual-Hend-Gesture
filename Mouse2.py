...
class Button:
    ...
    def draw(self, img, alpha=0.5):
        ...
        overlay = img.copy()
        rect_alpha = 1.0 if self.text in ["YES", "NO", "KBD", "DRAW"] else alpha
        ...
        if hand_count > 0:
                        ...
                        if time.time() - confirm_hover_start_time > HOVER_DURATION:
                            ...
                            if action_result == 'YES':
                                ...
                                if confirm_state == 'EXIT_KEYBOARD':
                                    ...
                                    keyboard_active = False
                                    typed_text = ""
                                    cv2.putText(desktop_canvas, "KEYBOARD EXITED & TEXT SAVED", (NOTEPAD_X, frame_height - 160), cv2.FONT_HERSHEY_DUPLEX, 1, (0, 255, 0), 2)
            ...
            if not is_drawing_mode_active and not keyboard_active:
                ...
                if len(lm_list) > 20:
                    ...
                    # --- Double & Single Click Detection (added) ---
                    if click_distance < CLICK_DISTANCE and confirm_state is None:
                        cv2.circle(desktop_canvas, (ix, iy), 15, (0, 255, 255), cv2.FILLED)

                        current_click_time = time.time()
                        # If last_click_time is recent enough -> double-click
                        if last_click_time != 0 and (current_click_time - last_click_time) <= DOUBLE_CLICK_INTERVAL:
                            # Double click: opens file/folder under cursor
                            pyautogui.doubleClick()
                            cv2.putText(desktop_canvas, "DOUBLE CLICK", (NOTEPAD_X, frame_height - 60),
                                        cv2.FONT_HERSHEY_DUPLEX, 1, (0, 255, 0), 2)
                            # Reset last_click_time so next pinch starts fresh
                            last_click_time = 0
                            last_action_time = current_click_time
                        else:
                            # Single click (or start of possible double-click)
                            # Enforce cooldown for single clicks
                            if current_click_time - last_action_time > COOLDOWN_TIME:
                                pyautogui.click()
                                cv2.putText(desktop_canvas, "SINGLE CLICK", (NOTEPAD_X, frame_height - 60),
                                            cv2.FONT_HERSHEY_DUPLEX, 1, (0, 255, 255), 2)
                                last_action_time = current_click_time
                            # store this click time to check for next one within DOUBLE_CLICK_INTERVAL
